...
tmux send-keys "python3 create_topomap.py --dt 1 --dir $2" Enter

# Change the directory to ../topomaps/bags and run the rosbag play command in the third pane
tmux select-pane -t 1
tmux send-keys "source /opt/ros/humble/setup.bash" Enter
tmux send-keys "cd /workspace/src/deployment/topomaps/bags" Enter
tmux send-keys "ros2 bag play -r 1.5 $1" # feel free to change the playback rate to change the edge length in the graph
